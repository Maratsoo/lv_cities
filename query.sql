...
-- 3. Atlasīt tikai nosaukumu, populacijas un novadu kolonnas (SELECT)
-- 4. Atlasīt pilsētas kuras ir jaunaks par 100 gadiem (WHERE)
-- 5. Atlasīt pilsētas ar lielak vai vienādas ar 5000 iedzīvotāju skaitu (WHERE)
...
SELECT title, population, region FROM cities;

-- 4

SELECT title, year, population, region FROM cities WHERE year > (YEAR(CURDATE()) - 100);

-- 5

SELECT title, population, region FROM cities WHERE population >= 5000;

-- 6

SELECT title, population, region FROM cities ORDER BY population DESC LIMIT 15;
...
SELECT region, SUM(population) AS total_population FROM cities GROUP BY region;
